...
from transformers import Data2VecAudioModel, Data2VecAudioPreTrainedModel

from src.models.wavlm_for_ctc import AttentionDistributionPredictor
        ...
        if self.layer_aggregation_method == "varan":
            ...
            for i in range(n_layers):
                if self.varan_separate_heads:
                    logits[i, :, :, :] = self.lm_head[i](asr_head_input[i, :, :, :])
                else:
                    logits[i, :, :, :] = self.lm_head(asr_head_input[i, :, :, :])
        else:
            logits = self.lm_head(asr_head_input)
