...
class VaranData2VecForSequenceClassification(Data2VecAudioForSequenceClassification):
    def __init__(
        self,
        config,
    ...
    def use_separate_classifiers(self):
        ...
        self.projector = nn.ModuleList(
            [
                nn.Linear(output_hidden_size, self.config.classifier_proj_size)
                for _ in range(self.config.num_hidden_layers)
            ]
        )
        for layer in self.projector[1:]:
            layer.weight.copy_(self.projector[0].weight)
            if layer.bias is not None:
                layer.bias.copy_(self.projector[0].bias)

        self.classifier = nn.ModuleList(
            [
                nn.Linear(self.config.classifier_proj_size, self.config.num_labels)
                for _ in range(self.config.num_hidden_layers)
            ]
        )
        for layer in self.classifier[1:]:
            layer.weight.copy_(self.classifier[0].weight)
            if layer.bias is not None:
                layer.bias.copy_(self.classifier[0].bias)
